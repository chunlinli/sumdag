...
peeling <- function(V, thresh) {
    source("../simulations/topological_order.R")
...
#' @param ref ref = T if users provide a reference panel; ref = F if all SNPs are pruned and standardized and no reference panel is needed
#' @param geno.ref.txt The location of the user-provided reference panel with the genotype of the q SNPs; only txt files are acceptable; The order SNPs needs to be matched with fullbetas and fullses.
#' @param thres The threshold of V used in the peeling algorithm.
#'
...
#' 
#' }
superDAG <- function(fullBeta, fullBetaSD, N, xtxdiag = 0, ref = F, geno.ref.text = NULL, thresh = 0.05) {
    ss_back <- summary_stats_preprocess(fullBetas, fullBetaSD, N, xtxdiag, ref, geno.ref.text)
    Vest <- estimateV_sum(fullBetas, fullBetaSD, N, ss_back)
    ...
    for (j in 1:ncol(PiHat)) {
        ...
        if (length(Jp) > 0) {
            ...
            if (length(Ap) > 0) {
                ...
                penBetas <- x1$beta
                # Obtaining psuedo AIC/BIC values
                x2 <- pseudoBIC.internal(penalizedBetas = penBetas, N = median(Nvec), xtx = XTX, ytyEst = 1, xtyEst = XTY)
            ...
            penBetas <- x1$beta
            # Obtaining psuedo AIC/BIC values
            x2 <- pseudoBIC.internal(penalizedBetas = penBetas, N = median(Nvec), xtx = YapYap, ytyEst = 1, xtyEst = XTY)
